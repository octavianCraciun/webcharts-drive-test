(function ($) {
    ...
    $.thumb.createSVGLayer = function (placeholder, eventHolder) {
        if ($('.flot-thumbs')[0]) {
            return $('.flot-thumbs')[0].firstChild;
        }
    ...
    $.thumb.createThumb = function (options) {
        ...
        var thumbIcon = document.createElementNS('http://www.w3.org/2000/svg', 'use');
        var thumbLabel = document.createElementNS('http://www.w3.org/2000/svg', 'text');
        var thumbGroup = document.createElementNS('http://www.w3.org/2000/svg', 'g');
        switch (opts.shape) {
            case 'left':
        ...
        opts.svgRoot.appendChild(thumbGroup);
    ...
    function extractTarget(evt) {
        ...
        if (elClass.indexOf('thumbIcon') !== -1) {
            target = evt.target.parentNode;
            elClass = target.getAttribute('class');
        }
        if (elClass.indexOf('thumbLabel') !== -1) {
            target = evt.target.parentNode;
            elClass = target.getAttribute('class');
        }
        return target;
...
(function ($) {
    ...
    function init(plot) {
        ...
        function updateSymbolSize() {
            ...
            constants.symbolTouchSize = parseInt(params);
        }
                        ...
                        cursor.thumbs
                            ...
                            .forEach(function () {
                                ...
                                if (targetCursor) {
                                    ...
                                    targetCursor.dragmode = xAxisThumb(cursor) ? 'x' : 'y';
        ...
        plot.hooks.shutdown.push(function (plot, eventHolder) {
            ...
            eventHolder.unbind('thumbmovestart', thumbmove.start);
            eventHolder.unbind('thumbmove', thumbmove.drag);
            eventHolder.unbind('thumbmoveend', thumbmove.end);
            plot.getPlaceholder().css('cursor', 'default');
        });
    }
    ...
    function mouseOverCursorManipulator(x, y, plot, cursor) {
        ...
        var grabRadius = constants.symbolTouchSize;
